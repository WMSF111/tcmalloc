...
// 获取CentralCache中一个非空的span（包含freelist链表）
SpanNode* CentralCache::GetOneSpan(SpanList& list, size_t byte_size)
{
	// 获取freeList不为空的span
	SpanNode* it = list.Begin(); // 获取_SpanList[index]桶的头Span
	while (it != list.End()) //如果该Span中节点数不为0
	{
	...
	// 如果没有找到非空的span，则创建一个新的挂有byte_size大小内存节点的Span
...
void CentralCache::ReleaseListToSpans(void* start, size_t byte_size) // 将start指向的对象链表释放到CentralCache的span中
{
	...
	_SpanList[index]._mutex.lock(); // 锁定对应的哈希桶
	while (start) //将MaxSize个对象的内存按MAP返回到CentralCache
	{
