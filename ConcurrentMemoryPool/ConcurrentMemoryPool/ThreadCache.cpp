...
// 从CentralCache中获取alignSize大小的内存块，将剩余内存块存入哈希桶index下标链表中
{
	//使用慢开始反馈调节
...
// 将list中MaxSize个对象存入CentralCache，并将其从自由链表中弹出，剩下的继续在自由链表中
{
	void* start = nullptr;
	void* end = nullptr;
	list.PopRange(start, end, list.MaxSize()); // 从自由链表中弹出MaxSize个对象的范围[start, end]
	CentralCache::GetInstance()->ReleaseListToSpans(start, size); // 将Start-->null的size大小的对象存入CentralCache
}
